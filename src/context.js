...
export class Context {
    ...
    static withTimeout(parent, timeoutMs) {
        return this.#newTimerContext(
            parent,
            timeoutMs,
            calcDeadlineMs(timeoutMs, Date.now()),
    ...
    static withTimeoutCause(parent, timeoutMs, cause) {
        return this.#newTimerContext(parent, timeoutMs, calcDeadlineMs(timeoutMs, Date.now()), cause, true);
    }
    ...
    #cancel(cause, isCustomCause) {
        ...
        // Record the cause only if it is custom and not already set
...
function calcDeadline(dtDeadline) {
    ...
    const timeoutMs = dtDeadline.getTime() - nowMs;
    return [timeoutMs, calcDeadlineMs(timeoutMs, nowMs)];
}

function calcDeadlineMs(timeoutMs, nowMs) {
